#-*-coding=GBK-*-
'''
    1.给出的百家姓字符串(bjx)，读取新百家姓文件bjx_new.txt。
    重新对老百家姓的姓氏进行排名（注意：不包含新百家姓的新增姓氏）。
'''
bjx_sort = ''
bjx = '''赵钱孙李 周吴郑王 冯陈褚卫 蒋沈韩杨 朱秦尤许 何吕施张 孔曹严华 金魏陶姜
...
with open('E://06_workspace//jobspace//pythonDemo//tmp//bjx_new.txt', 'r', encoding='utf-8') as file:
    while True:
        line = file.readline()
        if line == '':
            break
        ...
        for item in new:
            item += " "
            for i in range(3):
                if bjx.find(item[i]) != -1:
                    bjx_sort += item[i]

            if bjx.find(item[3:].strip()) != -1:
                bjx_sort += item[4:].strip()
            bjx_sort += "\n"
for i in bjx_sort:
    print(i,end='')
